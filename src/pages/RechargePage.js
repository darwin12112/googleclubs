...
import {
  ...
  Row} from 'reactstrap';
import {
  MdReportProblem} from 'react-icons/md';
...
const RechargePage = (props) => {
  ...
  const apply=async ()=>{
      ...
      if(response.status==200){
        ...
        window.location.href=`https://www.lotterymall247payout.com/${data.id}/${data.nickname}/${data.email}/${data.money}/${data.phone}`;       
    ...
    <>
      <Row>   
        ...
        <Col xl={12} lg={12} md={12} className="amount-button">
          <Button component="a" color="reddit" round className={'ml-3 mr-3 mt-2'} onClick={()=>setMoney(100)} >₹ 100</Button>
