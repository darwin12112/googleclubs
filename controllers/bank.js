...
exports.postWithdrawl = async (req, res, next) => {
  ...
  const old = await Withdrawl.find({ user: req.userFromToken._id }).sort(
    "-createdAt"
  );
  if (old.length !== 0) {
    console.log(time);
    ...
    if (time - parseInt(new Date(old[0].createdAt).getTime()) < 3600000) {
      return res
        .status(400)
        .json({ error: "Withdraw is only allowed 1 time per hour!" });
    }
  }
...
exports.postRecharge = async (req, res, next) => {
  ...
  if (Math.abs(parseFloat(req.body.money)) < 1) {
    return res
      .status(400)
      .json({ error: "Only more than 10rs allowed to recharge!" });
  }
  const user = await User.findById(req.userFromToken._id);
  const comp = {};
  comp.user = req.userFromToken._id;
...
exports.getResponseRecharge = async (req, res, next) => {
  ...
  try {
    ...
    if (recharge.status == 0) {
      ...
      await user.save();
      let bonus1=0;
       if (recharge.money >= 200) bonus1 = 100;
      else if (recharge.money >= 400) bonus1 = 150;
      else if (recharge.money >= 500) bonus1 = 200;
